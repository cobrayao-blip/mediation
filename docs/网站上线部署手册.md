...
- **ADMIN_DEFAULT_PASSWORD**：首次启动时若无用户，会自动创建管理员账号 **admin@mediation.local**，密码就是这里填的（登录后尽快在「用户中心」或管理后台修改）。
...
```bash
# 后端日志（数据库连接、接口报错等）
docker compose -f docker-compose.yml -f docker-compose.prod.yml -f docker-compose.port8080.yml logs backend

# 前端日志（Nginx 等）
docker compose -f docker-compose.yml -f docker-compose.prod.yml -f docker-compose.port8080.yml logs frontend
```
...
- **账号**：`admin@mediation.local`  
  （若你在 seed 或代码里改过默认邮箱，以实际为准。）
- **密码**：你在 `.env` 里设置的 **ADMIN_DEFAULT_PASSWORD**（未设置时一般为 `admin123`）。
...
# 查看最近日志（后端）
docker compose -f docker-compose.yml -f docker-compose.prod.yml -f docker-compose.port8080.yml logs --tail=100 backend

# 查看最近日志（前端）
docker compose -f docker-compose.yml -f docker-compose.prod.yml -f docker-compose.port8080.yml logs --tail=100 frontend

# 停止所有服务（数据保留在 Docker 卷中）
docker compose -f docker-compose.yml -f docker-compose.prod.yml -f docker-compose.port8080.yml down

# 再次启动（不重新构建）
docker compose -f docker-compose.yml -f docker-compose.prod.yml -f docker-compose.port8080.yml up -d
