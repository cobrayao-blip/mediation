...
**大模型（QWEN_API_KEY、DEEPSEEK_API_KEY）**：可留空。上线后在前端登录，在「配置密钥」里填写并保存即可，无需在 .env 中配置。
...
**必须带上 `docker-compose.port8080.yml`**，否则前端会占 80 端口，与主站 Nginx 冲突并报错 `port is already allocated`。
