...
const defaultScenarios = [
  {
    ...
    partyA: { name: "王大妈", trait: "守旧、固执、爱面子", background: "退休教师，独居，对自家的装修有着极高的情感寄托。" },
    partyB: { name: "李先生", trait: "焦躁、务实、防御性强", background: "私企中层，工作压力大，认为王大妈在敲诈勒索。" },
    sortOrder: 0,
  },
  {
    title: "小微企业货款清偿",
    ...
    partyA: { name: "陈总", trait: "悲愤、急功近利", background: "供应商，面临破产危机，对甲方极度不信任。" },
    partyB: { name: "赵总", trait: "逃避、世故、推诿", background: "采购方，确实经营困难，但习惯性拖延付款。" },
    sortOrder: 1,
  },
  {
    title: "老旧小区电梯加装",
    ...
    partyA: { name: "张爷爷", trait: "迫切、不善表达", background: "六楼住户，患有腿疾，加装电梯是其唯一的外出希望。" },
    partyB: { name: "刘女士", trait: "尖锐、维权意识极强", background: "一楼住户，从事法律相关工作，极其看重不动产价值。" },
    sortOrder: 2,
  },
];
...
export async function ensureSeed() {
  const [scenarioCount, skillCount] = await Promise.all([prisma.scenario.count(), prisma.skill.count()]);
  if (scenarioCount === 0) {
    await prisma.scenario.createMany({ data: defaultScenarios });
    console.log("已写入默认案例", defaultScenarios.length, "条");
  }
  
  // 更新技巧库：删除旧技巧，插入新技巧（确保使用最新的技巧数据）
  if (skillCount > 0) {
    // 检查是否需要更新（比较数量）
    if (skillCount !== defaultSkills.length) {
      console.log(`检测到技巧数量不一致（数据库：${skillCount}，种子数据：${defaultSkills.length}），正在更新...`);
      await prisma.skill.deleteMany({});
      await prisma.skill.createMany({ data: defaultSkills });
      console.log("已更新技巧库，写入", defaultSkills.length, "条技巧");
    } else {
      // 数量一致，但检查是否有新技巧（按名称检查）
      const existingNames = new Set((await prisma.skill.findMany({ select: { name: true } })).map(s => s.name));
      const newSkills = defaultSkills.filter(s => !existingNames.has(s.name));
      if (newSkills.length > 0) {
        console.log(`发现 ${newSkills.length} 个新技巧，正在添加...`);
        await prisma.skill.createMany({ data: newSkills });
        console.log("已添加新技巧");
      } else {
        console.log("技巧库已是最新版本，共", skillCount, "条技巧");
      }
    }
  } else {
    await prisma.skill.createMany({ data: defaultSkills });
    console.log("已写入默认技巧", defaultSkills.length, "条");
  }
}
